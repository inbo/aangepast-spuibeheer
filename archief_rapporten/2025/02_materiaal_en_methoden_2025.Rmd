...
```{r mapctdijzer, message=FALSE, warning=FALSE, fig.cap="Locaties van de ctd-divers"}
static.map("IJzer")
```

```{r mapctdkgo, message=FALSE, warning=FALSE, fig.cap="Locaties van de ctd-divers"}
static.map("Kanaal Gent-Oostende")
```

```{r mapctdakllk, message=FALSE, warning=FALSE, fig.cap="Locaties van de ctd-divers"}
static.map(c("Afleidingskanaal van de Leie","Leopoldskanaal"))
```

```{r mapctdne, message=FALSE, warning=FALSE, fig.cap="Locaties van de ctd-divers"}
...
Op de Noordede wordt de conductiviteit gemeten aan Maertensas, de Blauwe Sluis en Clemensheule die op 1.0, 3.0 en 4.0 km stroomopwaarts van de spuiconstructie liggen (Fig. \@ref(fig:mapctdne)).
