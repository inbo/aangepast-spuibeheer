clean_KGO <- function(file){
  ...
  data$open <- parse_date_time(paste(data$datum,data$open), c("ymd HM", "ymd H"), tz="Europe/Brussels")
  attr(data$open, "tzone") <- "GMT"
  data$dicht <- parse_date_time(paste(data$datum,data$dicht), c("ymd HM", "ymd H"), tz="Europe/Brussels")
  attr(data$dicht, "tzone") <- "GMT"
  data$datum <- NULL
  data$site = "KGO"
