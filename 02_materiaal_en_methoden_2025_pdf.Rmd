...
source("./code/functions/f.static.map.R")
output="pdf"
```
...
```{r ganzepoot, fig.cap = "Ligging van de spuiconstructies aan de Ganzepoot in de Ijzermonding", fig.show = "hold", out.width = "100%", fig.align = "center"}
include_graphics(path = "./media/ganzepoot.png")
```

### Sas Slijkens

Het Kanaal Gent-Brugge wordt samen met het Kanaal Brugge-Oostende aangeduid als KGO. In Brugge staat het Kanaal Brugge-Oostende in verbinding met het Kanaal Gent-Brugge, de Damse Vaart en het Boudewijnkanaal van Brugge naar Zeebrugge. Tussen Oudenburg en Oostende sluit het Kanaal Brugge-Oostende aan met het Kanaal Plassendale-Nieuwpoort. Het KGO bevindt zich deels in het bekken van de Brugse Polders en deels in het bekken van de Gentse kanalen. ’Sas Slijkens’ is de benaming voor de sluizen of sassen die het Kanaal Brugge-Oostende met de voorhaven van Oostende en de Noordzee verbinden (Fig. \@ref(fig:sas)). Het Sas Slijkens bezit twee spuisluizen met dubbele schuiven van ongeveer 4 m breed. Er zijn tevens vier spuikokers en een gronddam aanwezig. Debietmetingen in het KGO tussen Brugge en Oostende werden tot in 2020 gemeten en geregistreerd in Varsenare (Bron: HIC, Hydrologisch Informatie Centrum). Vanaf 2021 wordt het debiet gemeten in Oostkamp. Het gemiddelde debiet in Oostkamp in 2025 was 8.56 m³/s, terwijl het debiet varieerde tussen -9.42 m³/s en 38.57 m³/s. Op het kanaal wordt in functie van de binnenscheepvaart getracht een constant peil na te streven van 3.94 mTAW.

```{r sas, fig.cap = "Situering van het KGO, de Ringvaart in Brugge, de Damse Vaart (DV), het Boudewijnkanaal (BZ) en het Kanaal Plassendale-Nieuwpoort (PN). De locatie van de spuiconstructie ‘Sas Slijkens’ is rood omcirkeld (rechts onderaan).", fig.show = "hold", out.width = "100%", fig.align = "center"}
...
```{r AKLLK, fig.cap = "Situering van het Afleidingskanaal van de Leie (AKL) en het Leopoldkanaal (LK). Rood omcirkeld is de spuiconstructie van deze kanalen in Zeebrugge.", fig.show = "hold", out.width = "100%", fig.align = "center"}
...
```{r mapctdall, message=FALSE, warning=FALSE, fig.cap="Locaties van de ctd-divers"}
static.map("all")
```

```{r mapctdijzer, message=FALSE, warning=FALSE, fig.cap="Locaties van de ctd-divers in de Ijzer"}
static.map("IJzer")
```

```{r mapctdkgo, message=FALSE, warning=FALSE, fig.cap="Locaties van de ctd-divers in het kanaal Gent-Oostende"}
static.map("Kanaal Gent-Oostende")
```

```{r mapctdakllk, message=FALSE, warning=FALSE, fig.cap="Locaties van de ctd-divers in het Leopoldkanaal en het Afleidingskanaal van de Leie"}
static.map(c("Afleidingskanaal van de Leie","Leopoldskanaal"))
```

```{r mapctdne, eval=FALSE, message=FALSE, warning=FALSE, fig.cap="Locaties van de ctd-divers in de Noordede"}
static.map(c("Noordede"))
```
...
Vanaf 2016 werd aangepast spuibeheer toegepast. In principe wordt er aangepast gespuid in functie van glasaalmigratie tussen 1 maart en 15 mei 2025 op de Ijzer, het Kanaal Gent-Oostende, het Leopoldkanaal, het Afleidingskanaal van de Leie en de Noordede. In praktijk kan en wordt er veel minder aangepast gespuid.
